...
(defun index-pastes ()
  ...
  (let ((index (make-instance 'montezuma:index :path *index-path* :create-p T))
	(pastes (with-open-file (in *pastes-path* :direction :input) (cl:read in))))
      ...
      (time-form "Optimizing took ~,3F seconds."
		 (montezuma::optimize-index index))
      (montezuma:close-down index)
      (values)))
...
(defun load-index ()
  (setf *index* (make-instance
                 ...
                 :index-key "PASTES"
                 :name "LispPastes"
                 :title "Documents submitted to lisppastes"
                 ;; Unless otherwise specified, queries will search all fields simultaneously.
                 :default-field "*"
                 :fields '("date" "displaydate" "user" "channel" "number" "title" "contents")
                 :field-definitions
                 '(("date"
                    :stored nil :index :untokenized) ; :type :date)
                   ;;:form ((:year 4)-(:month 2)-(:day 2)))
                   ("displaydate"
                    :stored t :index :untokenized :type :date
                    :form ((:year 4)-(:month 2)-(:day 2)" "(:hour 2)":"(:min 2)":"(:sec 2)))
                   ("number" :stored t :index :untokenized :type :int)
                   ("user" :stored t :index :tokenized)
                   ("channel" :stored t :index :tokenized)
                   ("title" :stored t :index :tokenized)
                   ("contents" :stored t :index :tokenized)))))
...
;; (search-pastes "date:2006?05*" '(:num-docs 10000))
;; (search-pastes "date:2006?05*" '(:num-docs 10000) T)
;; (search-pastes "channel:lisp" '(:num-docs 10000) T)

#|
(defun annotation-apparatus (query)
  (unless *index*
    (load-index))
  (let ((*print-pretty* t))
    (montezuma::annotated-search-apparatus *index* query)))
|#
