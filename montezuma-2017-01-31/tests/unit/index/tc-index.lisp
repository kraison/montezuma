...
(defun do-test-index-with-table (index)
  (let ((data '((("def_field" . "one two"))
	...
	(add-document-to-index index
			       '(("field3" . "fool")))
        ;; queries need to reinstantiated after a change to the index.
        (setf query (make-instance 'wildcard-query :term (make-term "field3" "f*")))
...
(deftestfixture index-test
  ...
  (:testfun test-fs-index
   (let ((path1 (make-test-directory "fs-index/")))
      (flet ((delete-test-index ()
	       (cl-fad:delete-directory-and-files path1 :if-does-not-exist :ignore)))
	(delete-test-index)
	(condition-test
	 fs-index-1
	 (make-instance 'index
  ...
  (:testfun test-fs-index-is-persistent
    (let ((path2 (make-test-directory "persists/")))
      (flet ((delete-test-index ()
	       (cl-fad:delete-directory-and-files path2 :if-does-not-exist :ignore)))
	(delete-test-index)
...
;;	    (print (get-doc index 0))
  ...
  (:testfun test-persist-index
   (let ((path3 (make-test-directory "fsdir/persist/")))
     (cl-fad:delete-directory-and-files path3 :if-does-not-exist :ignore)
     (let ((index (make-instance 'index :default-field "f")))
       (let ((data '((("f" . "zero"))
                     (("f" . "one"))
                     (("f" . "two")))))
         (dolist (doc data)
           (add-document-to-index index doc)))
       (persist index path3 :create-p T)
      (test persist-index-1 (size index) 3)
      (test persist-index-2 (document-values (get-document index 0) "f")
	    "zero" #'string=)
      (close-down index))
     (let ((index (make-instance 'index :path path3)))
     ...
     (let ((index (make-instance 'index
       ...
       (test persist-index-6 (document-values (get-document index 0) "f")
             "romeo" #'string=)
       (let ((dir (make-fs-directory path3 :create-p NIL)))
         (persist index dir))
  ...
  (:testfun test-index-auto-update-when-externally-modified
   (let* ((path (make-test-directory "auto-update/"))
          (index (make-instance 'index
                                :path path
                                :default-field "f"
                                :create-p T)))
	...
	(test index-auto-update-when-externally-modified-4
	      (size index)
	      2)
	(let ((top-docs (search-index index (make-instance 'term-query :term (make-term "f" "content3")))))
	  (test index-auto-update-when-externally-modified-5 (size top-docs) 0)
	  (let ((iw (make-instance 'index-writer
	    ...
	    (close-down iw)
	    (let ((top-docs (search-index index (make-instance 'term-query :term (make-term "f" "content3")))))
	      (test index-auto-update-when-externally-modified-6 (size top-docs) 1)))))))
  (:testfun test-index-delete
	...
	(test index-delete-2 (size (search-index index (q :|id| "9"))) 1)
	(delete-document index 9)
	(test index-delete-3 (size (search-index index (q :|id| "9"))) 0)
	(test index-delete-4 (size (search-index index "id:8")) 1)
	(delete-document index "8")
	(test index-delete-5 (size index) 8)
	(test index-delete-6 (size (search-index index (q :|id| "8"))) 0)
	(test index-delete-7 (size (search-index index (qw :|cat| "/cat1*"))) 5)
	(query-delete index (qw :|cat| "/cat1*"))
	(test index-delete-8 (size index) 3)
	(test index-delete-9 (size (search-index index (qw :|cat| "/cat1*"))) 0)
	(close-down index))))
