  ...
  (:testfun test-fields-writer
   (let ((doc (make-instance 'document)))
	 ...
	 (let ((stored (read-vint fstream))
	       ...
	       (byte1 (read-byte-from-buffer fstream))
	 ...
	 (let ((field-num2 (read-vint fstream))
	       (byte2 (read-byte-from-buffer fstream))
...
(deftestfixture fields-reader
  ...
  (:testfun test-fields-doc
   (let ((dir (fixture-var 'dir))
	 ...
	 (write-byte-to-buffer fstream 0)
	 (write-string-to-buffer fstream "daily news")
	 (write-vint fstream 1)
	 (write-byte-to-buffer fstream 0)
	 (write-string-to-buffer fstream "Nothing happened today.")
	 (close-down fstream))
