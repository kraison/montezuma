...
(deftestfun test-standard-analyzer
  (with-input-from-string (input "D.Ba_l-n@gma-l.com AB&Sons Toys'r'us you're she's, #$%^$%*& job@dot I.B.M. the an AnD THEIR")
    (let* ((analyzer (make-instance 'standard-analyzer)) ; :language "english"))
	   (token-stream (token-stream analyzer "field" "english" input)))
      ...
      ;;(test standard-analyzer-4 (next-token token-stream) (make-token "you're" 37 43) #'token=) ; you're is a stop word
