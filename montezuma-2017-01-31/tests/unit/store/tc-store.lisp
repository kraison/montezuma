...
(defun do-test-basic-file-ops (dir)
  ...
  (delete-directory-file dir "tmpfile1")
...
(defun do-test-rename (dir)
  ...
  (let ((count-before (file-count dir)))
    (rename-directory-file dir "from" "to")
...
(defun do-test-rw-bytes (dir)
  (let ((bytes (vector #x34 #x87 #xf9 #xea #x00 #xff)))
    (do-rw-test dir bytes "byte" #'write-byte-to-buffer #'read-byte-from-buffer 6)))
...
(defun do-test-rw-strings (dir)
  (flet ((string* (str n)
	...
	(write-string-to-buffer ostream text)
	(write-string-to-buffer ostream (string* text 100))
	(close-down ostream))
...
(defun do-test-read-bytes (dir)
  (let ((str (string-to-bytes "0000000000"))
    ...
    (write-bytes-to-buffer ostream (string-to-bytes "how are you doing?") 18)
    (close-down ostream)
    (let ((istream (open-input dir "rw_read_bytes")))
      (read-bytes-from-buffer istream str 2 4)
      (atest read-bytes-from-buffer str (string-to-bytes "00how 0000") #'equalp)
      (read-bytes-from-buffer istream str 1 8)
      (atest read-bytes-from-buffer str (string-to-bytes "0are you 0") #'equalp)
      (close-down istream))))
