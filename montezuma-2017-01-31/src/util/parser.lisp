...
(defmethod make-special-grammar ((kind (eql '%)) sexp)
  (format t "making token-match-grammar for ~s~%" (second sexp))
...
(defgeneric code (grammar parser &key save-p)
  (:documentation "Emit the code to match the grammar"))
...
;; used production is translated into a local function with LABELS.
...
(defun tokenize-file (filename lexer-fn)
  ...
  (funcall lexer-fn (montezuma::file-contents filename)))
