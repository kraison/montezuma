...
(defun step2 (str)
  (let ((sfp (fill-pointer str)))
	...
	(case (char str (- (length str) 2))
	  ...
	  (#\e 
               (when (ends str "izer")    (r str "ize"  sfp)  (return))
               (when (ends str "er")      (r str "e"  sfp)     (return))
               )
...
(defun step3 (str)
  (let ((sfp (fill-pointer str)))
    (block nil
      (case (char str (1- (length str))) ; case last-letter
	(#\e (when (ends str "icate") (r str "ic" sfp) (return))
	     (when (ends str "ative") (r str "" sfp)   (return)) ; huh?
	     (when (ends str "alize") (r str "al" sfp) (return))
	     (when (ends str "able")  (r str "" sfp)   (return)))
        (#\h (when (ends str "eth")   (r str "" sfp)   (return));) ; REA: stem work-eth
             (when (ends str "ish")   (r str "" sfp)   (return))) ; REA damp-ish
	...
	(#\s (when (ends str "ness")  (r str "" sfp)   (return))) ; huh?
        (#\t (when (ends str "est")   (r str "" sfp)   (return))) ; REA: stem say-est
	)))
  str)
...
(defmethod stem ((word symbol) &optional (validate))
  (stem (string word) validate))

(defun last-try (str)
  (format t "stem last-try: ~a~%" str)
  (cond
   ((doublec str (1- (fill-pointer str))) (decf (fill-pointer str)))
   ((ends str "elv")                      (setto str "elf")) ; yourselves
   ((ends str "ie")                       (setto str "y"))
   ((ends str "i")                        (setto str "y"))
   ((ends str "e")                        (setto str ""))
   ((ends str "y")                        (setto str "ie")) ; blows out with non-extendable vector
   )
  str)

(defmethod stem ((str string) &optional (validate))
  (let ((len (length str))
        (inflected-word str))
    ...
    ;; algorithm.
    ;; VALIDATE is an optional function that should return T if the STR is a real word.
    (if (find-if-not #'alpha-char-p str)
        (return-from stem (values str "No letters for stemming.")))
    (if (<= len 2)
        (return-from stem (values str "At least 3 letters required for stemming."))) ; /*-DEPARTURE-*/
    ...
    (cond
     (validate
      (or 
       (funcall validate inflected-word)
       (funcall validate (step1ab str))
       (funcall validate (step1c str))
       (funcall validate (step2 str))
       (funcall validate (step3 str))
       (funcall validate (step4 str))
       (funcall validate (step5 str))
       (funcall validate (last-try str))))
     (t
      (step1ab str)
      (step1c str)
      (step2 str)
