...
(defclass priority-queue ()
  ((contents :type array)
   (max-size :type fixnum :initarg :max-size)   
   (predicate :type function :initarg :predicate :accessor predicate)
...
(defmethod initialize-instance :after ((queue priority-queue) &key)
  ;;(format t "initialize-instance priority-queue reverse-p ~s typeof ~s~%" (reverse-p queue) (type-of queue))
...
(defmethod less-than ((queue priority-queue) a b)
  (with-slots (predicate reverse-p) queue
    (if reverse-p
