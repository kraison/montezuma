...
(defgeneric all-field-names (document))

(defmethod all-field-names ((self document))
  (loop 
   for field in (all-fields self)
...
(defmethod (setf document-values) (value (document document) field-name)
  (let ((field (document-field document field-name)))
	...
	(add-field document (make-field field-name value)))))
#|
(defmethod search-field-value ((index index) (doc document))
  (fetch-field-value index doc (default-search-field index)))

(defmethod search-field-value ((index index) (keyvalue string))
  (search-field-value index (get-document-for-key index keyvalue)))

(defmethod search-field-value ((index index) (docnum integer))
  (search-field-value index (get-document index docnum)))
|#