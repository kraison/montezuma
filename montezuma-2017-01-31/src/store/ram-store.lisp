...
(defclass ram-directory (virtual-directory)
  ((dir :initarg :dir)
  ...
  (with-slots (dir) self
	...
	(let ((os (create-output self file))
	  ...
	  (let* ((len (size is))
	    ...
	    (read-bytes-from-buffer is buf 0 len)
	    (write-bytes-to-buffer os buf len)
	    (close-down is)
	    (close-down os))))
      (if close-dir-p
	  (close-down dir)))))
...
(defmethod delete-directory-file ((self ram-directory) name)
;;  (format T "~&~S" (list :delete (slot-value self 'id) name))
;;  (when (equal name "_3.tvf") (break))
  (with-slots (files) self
    (remhash (normalize-file-name name) files)))

(defmethod rename-directory-file ((self ram-directory) from to)
...
(defmethod close-down ((self ram-directory))
  )
...
(defmethod close-down :after ((self ram-index-output))
  (with-slots (file) self
    (setf (mtime file) (get-universal-time))))
...
(defmethod write-to ((self ram-index-output) output)
  ...
  (let ((file (slot-value self 'file))
	...
	(let ((len (if (= i last-buffer-number) last-buffer-offset buffer-size)))
	  (write-bytes-to-buffer output buffer len))))))
...
(defmethod close-down ((self ram-index-input))
