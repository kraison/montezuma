...
(defmethod write-byte-to-buffer ((self buffered-index-output) b)
  ;;(assert (typep b '(unsigned-byte 8)))
  ;;(unless b (break "write-byte-to-buffer ~s" b))
  (when (>= (slot-value self 'buffer-position) (slot-value self 'buffer-size))
    (flush self))
  (let ((buffer-position (slot-value self 'buffer-position)))
    (setf (aref (slot-value self 'buffer) buffer-position) (or b 0)) ; REA: convert NIL to zero: nil is still getting through
    (setf (slot-value self 'buffer-position) (+ buffer-position 1))))

(defmethod write-bytes-to-buffer ((self buffered-index-output) buffer length)
...
(defmethod clean-buffer ((self buffered-index-output) position)
  ;; this is a "no panic" response to a buffer filled with one or more nulls. 
  ;; Change every NIL to ZERO
  (with-slots (buffer buffer-size) self
    (loop
     for i from 0 to (min (1- buffer-size) position) do
     (unless (aref buffer i)
...
(defmethod flush ((self buffered-index-output))
  (with-slots (buffer buffer-position buffer-start) self
    (when (> buffer-position 0)
      (clean-buffer self buffer-position) ; change any NILs to zero
      (flush-buffer self buffer buffer-position)
      (incf buffer-start buffer-position)
      (setf buffer-position 0))))

(defmethod close-down ((self buffered-index-output))
  (flush self))
...
(defmethod read-byte-from-buffer ((self buffered-index-input))
  (let ((buffer-position (slot-value self 'buffer-position))
...
(defmethod read-bytes-from-buffer ((self buffered-index-input) buffer offset length)
  (with-slots (buffer-size buffer-start buffer-position buffer-length) self
    (if (< length buffer-size)
	(dotimes (i length)
	  (setf (aref buffer (+ i offset)) (read-byte-from-buffer self)))
	(let ((start (pos self)))
