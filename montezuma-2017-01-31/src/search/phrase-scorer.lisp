...
(defmethod score ((self phrase-scorer))
  (let ((raw (* (tf (similarity self) (freq self))
                (or (value self) 1))))
    ;; normalize
...
;;REA: still working on this code
(defmethod explain-score ((self phrase-scorer) reader doc-num doc-score)
  (let (;;(document (document doc-num))
        (explanation (make-explanation (doc-freq reader) doc-score))
        ;;(phrase_freq (doc-freq reader) 0.0)
        )
    ;;(setf (value explanation) @similarity.tf(phrase_freq)))
    ;;(setf (description explanation) "tf(phrase_freq=#{phrase_freq})")
    explanation))
