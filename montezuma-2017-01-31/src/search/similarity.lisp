...
(defparameter *norm-table* 
  (make-array 256 :initial-contents (loop for i from 0 below 256 collecting (byte315-to-float i))))

(defun similarity-byte-to-float (b)
  (assert (and (<= b 255) (>= b 0)))
  (aref *norm-table* b))
...
(defun similarity-float-to-byte (f)
  (cond ((<= f 0.0)
	...
	((>= f (aref *norm-table* 255))
	 255)
	(T
         (loop
          for i from 0 below 256
          if (>= (aref *norm-table* i) f)
          return i))))
