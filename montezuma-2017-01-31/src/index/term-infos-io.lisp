...
(defmethod initialize-instance :after ((self term-infos-writer) &key directory segment)
  (with-slots (out is-index index-interval skip-interval) self
    ...
    (unless is-index
      (with-slots (other field-infos) self
...
(defmethod close-down ((self term-infos-writer))
...
(defmethod close-down ((self term-infos-reader))
  (with-slots (orig-enum index-enum) self
    (when orig-enum
      (close-down orig-enum))
    (when index-enum
      (close-down index-enum))))
...
(defmethod get-position ((self term-infos-reader) term)
  (with-slots (size) self
    (if (= size 0)
	-1
	(let ((ind-offset (get-index-offset self term)))
