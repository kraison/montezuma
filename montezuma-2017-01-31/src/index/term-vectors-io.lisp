...
(defmethod close-down ((self term-vectors-writer))
...
(defmethod close-down ((self term-vectors-reader))
  ;; FIXME handle errors?
  (with-slots (tvx tvd tvf) self
    (close-down tvx)
    (close-down tvd)
  ...
  (with-slots (tvf tvf-format) self
    ...
    (let ((num-terms (read-vint tvf)))
	  ...
	  (let ((store-positions NIL)
		...
		(let ((bits (read-byte-from-buffer tvf)))
