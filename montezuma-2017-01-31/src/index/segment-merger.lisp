...
(defgeneric merge-segment (segment-merger))

(defmethod merge-segment ((self segment-merger))
...
(defmethod merge-vectors ((self segment-merger))
  (with-slots (directory segment readers field-infos) self
	...
	(close-down term-vectors-writer)))))
  ...
  (with-slots (directory segment readers freq-output prox-output term-infos-writer
    ...
    (unwind-protect
	 (progn
      ...
      (when freq-output (close-down freq-output))
      (when prox-output (close-down prox-output))
      (when term-infos-writer (close-down term-infos-writer))
      (when queue (close-down queue)))))
	    ...
	    (unwind-protect
		 (dosequence (reader readers)
		   (let* ((max-doc (max-doc reader))
			 ...
			 (write-byte-to-buffer output (aref input k))))))
	      (close-down output))))))))
