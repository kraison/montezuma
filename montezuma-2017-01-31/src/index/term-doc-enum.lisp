...
(defmethod do-seek ((self segment-term-doc-enum) term-info)
  (with-slots (counter doc-freq  doc skip-doc skip-count num-skips freq-pointer
...
(defmethod close-down ((self segment-term-doc-enum))
  (with-slots (freq-stream skip-stream parent) self
    (when freq-stream
      (close-down freq-stream))
    (setf freq-stream nil)
    (when skip-stream
      (close-down skip-stream)
...
(defmethod next? ((self segment-term-doc-enum))
  (with-slots (counter doc-freq freq-stream freq deleted-docs doc) self
  (while T
    (when (eql counter doc-freq)
...
(defmethod read-segment-term-doc-enum ((self segment-term-doc-enum) docs freqs 
                                       &optional (start 0))
  (with-slots (doc-freq freq freq-stream doc counter deleted-docs) self
    (let ((i start)
	  (needed (length docs)))
      (while (and (< i needed) (< counter doc-freq))
        (let ((doc-code (read-vint freq-stream)))
	  (incf doc (ash doc-code -1))
	  (if (logbitp 0 doc-code)
	      (setf freq 1)
...
(defmethod skip-to ((self segment-term-doc-enum) target)
  (with-slots (doc-freq counter doc skip-interval skip-stream skip-doc freq-pointer freq-stream
	...
	(setf have-skipped T))
      (let ((last-skip-doc skip-doc)
	    ...
	    (last-prox-pointer -1)
	    (num-skipped (- -1 (mod counter skip-interval))))
...
(defmethod close-down :after ((self segment-term-doc-pos-enum))
  (with-slots (prox-stream) self
    (close-down prox-stream)))
