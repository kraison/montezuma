  ...
  (with-slots (segment index-directory deleted-docs field-infos fields-reader
	...
	(setf dir cfs-reader))
      ...
      (when (has-deletions-p info)
        ;; We assume here that the write lock is held
	(setf deleted-docs
...
(defun get-segment-reader (segment-info &key infos close-directory-p id)
  (make-instance 'segment-reader
		 :directory (segment-directory segment-info)
		 :info segment-info
                 :id id
		 :segment-infos infos
...
(defmethod do-commit ((self segment-reader))
  (with-slots (segment deleted-docs-dirty-p deleted-docs norms norms-dirty-p segment-reader
    ...
    (when (and undelete-all-p
	       (file-exists-p index-directory (add-file-extension segment "del")))
      (delete-directory-file index-directory (add-file-extension segment "del")))
    (when norms-dirty-p
      (loop for norm being the hash-values in norms
	 do (when (dirty-p norm)
	      (re-write norm index-directory segment (max-doc self) cfs-reader))))
    (setf deleted-docs-dirty-p NIL
	  norms-dirty-p NIL
	  undelete-all-p NIL)))
...
(defmethod do-close ((self segment-reader))
  (with-slots (fields-reader term-infos freq-stream prox-stream
    ...
    (close-down fields-reader)
    (close-down term-infos)
    (when freq-stream (close-down freq-stream))
    (when prox-stream (close-down prox-stream))
    (close-norms self)
    (when tv-reader-orig (close-down tv-reader-orig))
    (when cfs-reader (close-down cfs-reader))))
	...
	(if (not (null (bytes norm)))
	    ...
	    (let ((norm-stream (clone (input-stream norm))))
	      (unwind-protect
		   (progn
		     (seek norm-stream 0)
		     (read-bytes-from-buffer norm-stream bytes offset max-doc))
		(close-down norm-stream)))))))
...
(defmethod close-norms ((self segment-reader))
  (loop for norm being the hash-values in (slot-value self 'norms)
       do (close-down (input-stream norm))))
...
(defmethod re-write ((self norm) directory segment count cfs-reader)
  (let ((out (open-segment-file directory segment "tmp" :output)))
    (unwind-protect
	 (write-bytes-to-buffer out (bytes self) count)
      (close-down out))
