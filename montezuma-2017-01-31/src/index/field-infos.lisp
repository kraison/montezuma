...
(defmethod get-field ((self field-infos) index)
  (if (integerp index)
      (if (or (eql index +not-a-field+)
              (< index 0)
              (> index (length (slot-value self 'fi-array) )))
...
(defmethod write-to-dir ((self field-infos) dir name)
  (let ((output (create-output dir name)))
    (unwind-protect
	 (write-infos self output)
      (close-down output))))

(defgeneric write-infos (field-infos output))

(defmethod write-infos ((self field-infos) output)
  (write-vint output (size self))
  (dotimes (i (size self))
    (let ((fi (get-field self i)))
      (write-string-to-buffer output (field-name fi))
      (write-byte-to-buffer output (get-field-info-byte fi)))))

(defgeneric read-infos (field-infos input))

(defmethod read-infos ((self field-infos) input)
  (let ((size (read-vint input)))
    (dotimes (i size)
      (let* ((name (read-string input))
	     (bits (read-byte-from-buffer input))
