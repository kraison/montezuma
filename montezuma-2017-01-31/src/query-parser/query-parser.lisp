...
(alexandria:define-constant brief-explanation-of-query-syntax
"
...
(alexandria:define-constant punctuation-character
    '(#\( #\) #\" #\* #\? #\: #\^ #\[ #\] #\{ #\} #\- #\+ #\! #\~ #\/ #\, #\!)
  :test 'equalp)
...
(defmethod $get-term-query ((parser query-parser) word)
  (do-multiple-fields (field-name parser)
    ;;(break "$get-term-query field ~s word ~s" field-name word)
    (let ((field-type (field-definition-type parser field-name))
          (field-definition (field-definition parser field-name)))
...
|#
        (float
         (make-instance 'float-term-query :term (make-term field-name word) :definition field-definition))
        (date
         ;; we have a field defined to be a date so now let's parse our term and make a query
         (let ((date (parse-query-date word)))
           (if date
               (make-instance 'date-term-query :term (make-term field-name date) :definition field-definition))))
        (t
         (let ((tokens (all-tokens (analyzer parser) field-name (language parser) (resolve-entities word))))
           ;;(break "$get-term-query tokens: ~s" tokens)
           (cond
            ((zerop (length tokens))
             (make-instance 'term-query :term (make-term field-name "")))

            ((= (length tokens) 1)
             (make-instance 'term-query :term (make-term field-name (term-text (car tokens)))))

            (t
             (let ((pq (make-instance 'phrase-query)))
               (dolist (token tokens)
                 (add-term-to-query pq (make-term field-name (term-text token)) nil (token-increment token)))
               pq)))))))))
...
(defprod field-specifier ()
  ...
  (/ (^ (word ":")
        ;(break "field-specifier ~a" word)
        (let ((vfld (validate-field-name parser word)))
          (if vfld
              ($set-query-field parser vfld)
            (error 'no-matching-fields
                   :query (query parser)
                   :message "No fields matching"
                   :field word
                   :fields (field-definitions parser)))))
     (^ (wild-word ":")
