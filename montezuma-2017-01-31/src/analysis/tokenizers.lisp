...
(defmethod close-down ((self tokenizer))
...
(defmethod token-regexp ((self regexp-tokenizer))
  ;;(cl-ppcre:create-scanner "[\\w\\-\\*\\?']+" :multi-line-mode T)) ; match a sequence of word characters
  (cl-ppcre:create-scanner "\\w+" :multi-line-mode T)) ; match a sequence of word characters
...
(defmethod token-regexp ((self whitespace-tokenizer))
  (cl-ppcre:create-scanner "\\S+" :multi-line-mode T)) ; match any non-whitespace sequence

(defclass word-tokenizer (regexp-tokenizer)
  ())

(defmethod token-regexp ((self word-tokenizer))
  (cl-ppcre:create-scanner "[a-zA-Z0-9.\\-@]+" :multi-line-mode T))


(defclass text-tokenizer (regexp-tokenizer)
  ())

(defmethod token-regexp ((self text-tokenizer))
  (cl-ppcre:create-scanner "[a-zA-Z0-9]+|\\W" :multi-line-mode T))
