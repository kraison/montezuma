...
(defmethod step1a ((z stemmer))
  (with-slots (str j) z
    (when (ends z "ied")
      (setf (char str (1- (fill-pointer str))) #\s)) ;setto z "s"))
    (cond
     ((ends z "ies")  
      ;;(format t "step1a ies ~s" z)
      (setto z (if (>= j 1) "i" "ie")))
    ...
    (loop
     ...
     (when (ends z suffix)
       ...
       (when (vowelinstem z)
         ...
         ;;(setto0 z)
         ;;(format t "~%step1b after setto0 ~s~%" z)
         (loop
            ...
            (let ((z (stem input)))
              (cond
               ...
               (t
                ...
                (if (or (eq abort-on-failure t) 
                        (and (numberp abort-on-failure) (> failures abort-on-failure)))
                    (return))
