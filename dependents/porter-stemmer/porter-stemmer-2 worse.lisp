...
(defmethod vowelinstem ((z stemmer))
  (with-slots (str j) z
    (find-if #'is-a-vowel str :end (1+ j))))
  ...
  (with-slots (str j p1) z
    (loop
     ...
     (when (ends z suffix)
       ...
       ;;(format t "~%step1b ~s~%" z)
       (when (vowelinstem z)
         ;;(format t "~%step1b passed vowelinstem ~s~%" z)         
         (setto0 z)
         (loop
         ...
         ;;(format t "~%step1b test doublec ~s" z)

         (cond 
          ((doublec str) (decf (fill-pointer str)))
          (t
           ;;(format t "~%step1b ~s test (= j p1) ~s (shortv z) ~s" z (= j p1) (shortv z))
           (when (and (= j p1) (shortv z))
             (setto z "e"))))))))
  z)
#|
(defmethod step1b ((z stemmer))
    ...
    (loop
     ...
     (when (ends z suffix)
       ...
       (when (vowelinstem z)
         ...
         (format t "step1b ~s shortv ~s~%" z (shortv z))
         (cond 
          ((doublec str) (decf (fill-pointer str)))
          ((and (= j p1)
                (shortv z))
  ...
  (with-slots (str j p1) z
    (loop
     ...
     (when (ends z suffix)
       ...
       ;;(format t "~%step1b ~s~%" z)
       (when (vowelinstem z)
         ;;(format t "~%step1b vowelinstem ~s~%" z)
         (setto0 z)
         (loop
          as (suffix replacement) in '(("at" "ate") ("bl" "ble") ("iz" "ize")) do
          (when (ends z suffix)
...
(defmethod step5 ((z stemmer))
  (with-slots (str j) z
    (cond
     ((and (ends_ch z #\e)
           (or (R2 z) (and (R1 z) (not (short-syllable str j)))))
      ;;(decf (fill-pointer str))
      (setto0 z))
     ((and (ends_ch z #\l)
           (R2 z) 
            ...
            (let ((z (stem input)))
              (cond
               ...
               (t
                ...
                (if (and max-failure 
                        (and (numberp max-failure) (> failures max-failure)))
                    (return))
