...
(defun Porter3a (inflected-word validator)
  (let* ((str (make-dynamic-string inflected-word))
         ...
         (entry nil)
         (lexeme #'(lambda(word)
                     ; don't call the validator if we've already rejected this word.
                     (when (string/= last-word word)
                       (setf last-word (copy-seq word))
                       (funcall validator word)))))
    (if (find-if 
         #'(lambda(step) (setf entry (funcall lexeme (funcall step str))))
         '(prelude step1ab step1c step2 step3 step4 step5 final-step[1] final-step[2]))
        (make-instance 'lexeme :word (word entry) :base (base entry) :analysis :stemmed))))
...
(defun stem-to-lexeme (str &optional (validator #'is-a-word))
  ;; Execute Porter 3 stemming to get the first valid stem
  (let ((inflected-word (string-downcase str)))
...
(defun Porter3 (str &optional (validator #'is-a-word))
  ;; Execute Porter 3 stemming to get a lexeme
  (get-lexeme (stem-to-lexeme str validator)))
...
(defun time-check (&key (algorithm #'porter3) (wordlist (collect-words)) (repeats 10000))
  (time
     (dotimes (n repeats)
       (dolist (wordin wordlist)
         (funcall algorithm wordin))))
  (* repeats (length wordlist)))

(defun timings ()
  (load (compile-file *porter-source-file*))
  (dolist (file '(("word-list.txt" 10000) ("porter-stemmer-input.txt" 50)))
    (let* ((path (merge-pathnames (car file) *porter-data-directory*))
           (wordlist1 (collect-words path))
           (repeats (cadr file)))
      (format t "~%timings: ~a Porter1 repeats: ~d~%" (car file) repeats)
      (time-check :algorithm #'stem :repeats repeats :wordlist wordlist1)
      (format t "~%timings: ~a Porter3 repeats: ~d~%" (car file) repeats)
      (time-check :algorithm #'porter3 :repeats repeats :wordlist wordlist1))))
...
(defun count-words (str &key (start 0)
  ...
  (let ((n 0))
    (map-words str #'(lambda(&rest args) (declare (ignore args)) (incf n)) :start start :end end :letterp letterp)
    n))
