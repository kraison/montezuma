...
(defclass stemmer ()
  ((str :initform nil :accessor stemmer-str :initarg :str)   ;; b buffer for word to be stemmed
   ...
   (p2 :initform 0 :accessor stemmer-p2 :initarg :p2)
   (y-found :initform nil :accessor y-found :initarg :y-found)
   (reversed :initform nil :accessor reversed :initarg :reversed)
   (line-count :initform 0 :accessor line-count :initarg :line-count)
   (inflected-form :initform "" :accessor inflected-form :initarg inflected-form)
   (expected :initform "" :accessor expected :initarg :expected)))
...
(defmethod step1b ((z stemmer))
  (with-slots (str j p1) z
    ...
    (when (vowel-in-stem str j)
      (format t "~%step1b ~s vowel-in-stem ~s" z (vowel-in-stem str j))
      (setto0 z)
      (loop
  ...
  (with-slots (str j p1) z
    ;;(unless (zerop (length str))
      (loop
        ...
        (when (ends z suffix)
          ;;(format t "~%?step2 ~s suffix ~s R1 ~s~%" z suffix (r1 z))
          (if (>= j p1) ;;(R1 z)
              (setto z replacement))
          (return-from step2 z)))
      (cond
...
(defmethod mark-regions ((z stemmer))
  (with-slots (str j p1 p2) z
    (let ((syllable_count 0)
          (ch_prev_vowel nil)
          (start 0))
      (setf p1 (length str) p2 (length str))
      (when (exception_p1 z)
        (setf syllable_count 1 start p1))
      (loop
        for i from start to (1- j)
        as ch = (char str i)
          ...
          (multiple-value-bind (input expected) (split-line line)
            (let ((z (stem input)))
              (cond
               ...
               (t
                (incf failures)
                (setf (reversed z) (reverse input))
                (setf (line-count z) line-count)
                (setf (inflected-form z) input)
                (setf (expected z) expected)
                (push z failure-list)
;;                (format t "[~d] (stem ~s) => ~s wanted ~s~%" line-count input (stemmer-str z) expected)
    ...
    (setf failure-list (sort failure-list #'string< :key #'reversed))
    (dolist (z failure-list)
      (format t "[~d] (stem ~s) => ~s wanted ~s~%"
        (line-count z)
        (inflected-form z)
        (stemmer-str z)
        (expected z)))
      
    (format t "sucesses: ~d failures: ~d failure rate: ~d%~%" successes failures (* 100 (FLOAT (/ failures (+ successes failures)))))
    failures))
