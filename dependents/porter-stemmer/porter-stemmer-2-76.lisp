...
(defun short-syllable (str j)
  ;;(format t "~%short-syllable ~s j ~d chj ~s ch-1 ~s~%" str j (char str j) (and (> j 1) (char str (1- j))))
  (decf j) ; try this
  (cond 
   ((and (> (length str) 1)
         (vowelp str 0)
     ...
     (when (ends z suffix)
       ...
       (when (vowelinstem z)
         ...
         (cond 
          ...
          ((and (= j p1))
           (format t "~%step1b shortv ~s shortv ~s~%" z (shortv z))
           (when (shortv z)
             (setto z "e"))))))))
  z)
...
(defmethod valid_li_ending ((z stemmer))
  (with-slots (str j) z
    ;;(break "z ~s ch[~d] ~s" z j (char str (1- j)))
    (if (> j 0)
            ...
            (let ((z (stem input)))
              (cond
               ...
               (t
                ...
                (if (and (numberp max-failures) (> failures max-failures))
                    (return))
